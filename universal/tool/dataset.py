...
# 来根据索引序号获取图片和标签, 有__len__(self)函数来获取数据集
    def __init__(self, alphabet, test=False, root=None, transform=None, reverse=False, ifRotate90=False):
        self.env = lmdb.open(
            root,
            max_readers=1,
            readonly=True,
    ...
    def __getitem__(self, index):
        ...
        with self.env.begin(write=False) as txn:
            ...
            label = ''.join(label[i] if label[i] in self.alphabet else '' 
        ...
        return (img, label, very_high_flag)
